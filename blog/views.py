...
def index(request):
    popular_posts = Post.objects.annotate(likes_count=Count("likes")).prefetch_related(
        "author"
    )
    ...
    popular_tags = Tag.objects.annotate(related_posts_count=Count("posts")).order_by(
        "-related_posts_count"
    )
